...
note: for rollup the sources list will be in better shape
*/
...
function origStackTrace(x: StackFrame): string {
  return (
    ((x.getFileName() || x.getScriptNameOrSourceURL()) + ':' + x.getLineNumber() + ':' + x.getColumnNumber())
      // ' - ' +
      // x.getTypeName() +
      // '.' +
      // x.getFunctionName() +
...
function prepareStackTrace(error: Error, stack: StackFrame[]) {
  ...
  for (let i = stack.length - 1; i >= 0; i--) {
    processedStack.push('\n    at ' + wrapCallSite(stack[i], state) + ' (' + origStackTrace(stack[i]) + ')');
